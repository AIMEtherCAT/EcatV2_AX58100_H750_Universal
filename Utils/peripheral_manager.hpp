...
public:
    explicit UartPeripheral(const osMutexId mutex, UART_HandleTypeDef *huart, Buffer *send_buf,
                            Buffer *recv_buf) : Peripheral(mutex), _huart(huart),
                                                              _send_buf(send_buf),
                                                              _recv_buf(recv_buf) {
    }
...
public:
    explicit USART1Peripheral(const osMutexId mutex, UART_HandleTypeDef *huart, Buffer *send_buf,
                              Buffer *recv_buf) : UartPeripheral(
...
public:
    explicit UART4Peripheral(const osMutexId mutex, UART_HandleTypeDef *huart, Buffer *send_buf,
                              Buffer *recv_buf) : UartPeripheral(
        mutex, huart, send_buf, recv_buf) {
    }
...
public:
    explicit UART8Peripheral(const osMutexId mutex, UART_HandleTypeDef *huart, Buffer *send_buf,
                              Buffer *recv_buf) : UartPeripheral(
        mutex, huart, send_buf, recv_buf) {
    }
...
Peripheral *get_peripheral(PeripheralType type);
