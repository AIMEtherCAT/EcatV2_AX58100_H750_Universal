...
static std::map<PeripheralType, std::unique_ptr<Peripheral> > instances;
...
void init_peripheral_manager() {
    ...
    instances[PeripheralType::PERIPHERAL_USART1] = std::make_unique<USART1Peripheral>(USART1InitMutexHandle,
        &huart1, get_buffer(BufferType::USART1_SEND), get_buffer(BufferType::USART1_RECV));
    instances[PeripheralType::PERIPHERAL_UART4] = std::make_unique<UART4Peripheral>(UART4InitMutexHandle,
        &huart4, get_buffer(BufferType::UART4_SEND), get_buffer(BufferType::UART4_RECV));
    instances[PeripheralType::PERIPHERAL_UART8] = std::make_unique<UART8Peripheral>(UART8InitMutexHandle,
        &huart8, get_buffer(BufferType::UART8_SEND), get_buffer(BufferType::UART8_RECV));
    instances[PeripheralType::PERIPHERAL_I2C3] = std::make_unique<I2C3Peripheral>(I2C3InitMutexHandle);
    instances[PeripheralType::PERIPHERAL_TIM2] = std::make_unique<TIM2Peripheral>(TIM2InitMutexHandle);
    instances[PeripheralType::PERIPHERAL_TIM3] = std::make_unique<TIM3Peripheral>(TIM3InitMutexHandle);
    instances[PeripheralType::PERIPHERAL_CAN] = std::make_unique<CANPeripheral>(CANInitMutexHandle);
}

Peripheral *get_peripheral(const PeripheralType type) {
