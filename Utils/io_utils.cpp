...
namespace aim::io {
    ...
    namespace big_endian {
        float read_float16(const uint8_t *buf, int *offset) {
            const uint16_t h = static_cast<uint16_t>(buf[*offset + 1]) | static_cast<uint16_t>(buf[*offset]) << 8;
            *offset += 2;
        ...
        float read_float(const uint8_t *buf, int *offset) {
            float res;
            auto *p = reinterpret_cast<uint8_t *>(&res);
            for (int i = 0; i < 4; i++) p[3 - i] = buf[*offset + i];
            *offset += 4;
            return res;
        }
        ...
        uint32_t read_uint32(const uint8_t *buf, int *offset) {
            uint32_t res = 0;
            for (int i = 0; i < 4; i++) res |= static_cast<uint32_t>(buf[*offset + i]) << (8 * (3 - i));
            *offset += 4;
            return res;
        }

        int32_t read_int32(const uint8_t *buf, int *offset) { return static_cast<int32_t>(read_uint32(buf, offset)); }

        uint16_t read_uint16(const uint8_t *buf, int *offset) {
            const uint16_t res = buf[*offset] << 8 | buf[*offset + 1];
            *offset += 2;
            return res;
        }

        int16_t read_int16(const uint8_t *buf, int *offset) { return static_cast<int16_t>(read_uint16(buf, offset)); }

        uint8_t read_uint8(const uint8_t *buf, int *offset) { return buf[(*offset)++]; }

        void write_uint8(const uint8_t value, uint8_t *buf, int *offset) { buf[(*offset)++] = value; }
