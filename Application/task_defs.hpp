...
namespace aim::ecat::task {
    ...
    namespace dd_motor {
        struct MotorReport {
            ...
            ThreadSafeValue<int16_t> rpm{};
            ThreadSafeValue<int16_t> current{};
            ThreadSafeValue<uint8_t> mode{};
            ThreadSafeValue<uint8_t> error{};
            ThreadSafeTimestamp last_receive_time{};
        };

        enum class CtrlMode : uint8_t {
            OPEN_LOOP_VOLTAGE = 0x00,
            CLOSED_LOOP_CURRENT = 0x01,
            SPEED = 0x02,
            SINGLE_ROUND_POSITION = 0x03
        };
        ...
        class DD_MOTOR final : public CanRunnable {
        public:
            explicit DD_MOTOR(buffer::Buffer *buffer);
        ...
        private:
            ...
            Motor motors_[4]{};
            int16_t cmds_[4]{};
            uint8_t mode_change_packet[8]{};
            uint8_t feedback_mode_change_packet[8]{};
            uint32_t can_cmd_packet_id_{};
            uint8_t enabled_motors_{};
        };
    }
}
